...
この記事では、実際に動かしている GLSL シェーダーを分解しながら、屈折ガラスの作り方をひとつずつ説明します。最後に、パラメータをいじってコードを書き出せるツールにした話もします。
...
SDF と勾配が分かれば、丸・ピル・カードなど形を変えるのも `sdRoundRect` の引数を変えるだけです。ガラス以外（水滴・凸レンズなど）にも応用が効くので、ぜひ手元の画像で色々試してみてください。
