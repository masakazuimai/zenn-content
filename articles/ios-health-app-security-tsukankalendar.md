---
title: "個人開発iOSヘルスケアアプリのセキュリティ設計 ― 痛風管理アプリで実践した7つの防御策"
emoji: "🛡️"
type: "tech"
...
この記事では、約2,600行・18ファイルのSwiftUIアプリで実践した**7つのセキュリティ設計判断**を、実コードとともに解説します。
...
完璧なアプリはありません。現時点で認識している改善余地を透明に共有します。

### CSVエクスポートのインジェクション対策

CSVファイルをExcelで開く際、セルの先頭が`=`や`@`だと数式として実行される可能性があります（CSV Injection）。ユーザーが自由入力できるフィールドがある場合は、エスケープ処理を入れるべきです。
...
UserDefaultsに保存されるデータは平文です。iOSのデータ保護（Data Protection）により端末ロック時は暗号化されますが、アプリレイヤーでの追加暗号化は行っていません。

健康データの機密性を考慮すると、将来的にはKeychainやCryptoKitによる暗号化が望ましいです。
